...
benchmark :
	...
	poetry build
	if [ ! -d "benchmark/.venv" ]; then python3 -m venv benchmark/.venv; fi
	benchmark/.venv/bin/pip install -r benchmark/requirements.txt \
	&& benchmark/.venv/bin/pip install --force --find-links=dist typical
	benchmark/.venv/bin/pytest benchmark
