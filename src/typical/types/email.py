...
class EmailAddrInfo:
    ...
    Notes:
        An email address is also, technically, a network address. However, it has some
        more constraints, and we're interested in only a specific amount of information.
    """
    ...
    """The fully-qualified email address, as string."""
    address_encoded: str = dataclasses.field(init=False)
    """The fully-qualified email address, as string, url-encoded."""
    is_named: bool = dataclasses.field(init=False)
    """Whether this email has a name prefix, also known as 'pretty'."""
    is_private: bool = dataclasses.field(init=False)
    """Whether the hostname points to a private IP address."""
    is_internal: bool = dataclasses.field(init=False)
    """Whether the hostname points to an internal IP address."""

    def __post_init__(self):
        object.__setattr__(self, "address", self._get_address())
        object.__setattr__(self, "address_encoded", self._get_address_encoded())
        object.__setattr__(self, "is_named", self._get_is_named())
...
class Email(NetworkAddress):
    ...
    Examples:
        >>> import typical
        >>> email = typical.Email("Foo Bar <foo.bar@foobar.net>")
        >>> print(email)
        Foo Bar <foo.bar@foobar.net>
        >>> email.info.host
        'foobar.net'
    ...
    info: EmailAddrInfo  # type: ignore[assignment]

    def _getinfo(self) -> EmailAddrInfo:  # type: ignore[override]
