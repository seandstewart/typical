...
from typical.types import secret
...
class NetAddrInfo:
    ...
    """The fully-qualified network address, url-encoded."""
    query: Mapping[str, list[str]] = dataclasses.field(init=False, hash=False)
    """The query-string, parsed into a mapping of key -> [values, ...]."""
    parameters: Mapping[str, list[str]] = dataclasses.field(init=False, hash=False)
    """The parameters-string, parsed into a mapping of key -> [values, ...]."""
    is_default_port: bool = dataclasses.field(init=False)
    """Whether address is using the default port assigned to the given scheme."""
    is_relative: bool = dataclasses.field(init=False)
    """If true, indicates the address *does not contain* host information."""
    is_absolute: bool = dataclasses.field(init=False)
    """If true, indicates the address *does contain* host information."""
    is_private: bool = dataclasses.field(init=False)
    """Whether the given host points to a private IP address."""
    is_internal: bool = dataclasses.field(init=False)
    """Whether the given host points to an internal IP address."""
    ...
    def from_str(cls, value) -> "NetAddrInfo":
        ...
        return cls(
            scheme=scheme,
            auth=match["auth"] or "",
            password=secret.SecretStr(match["password"] or ""),
            host=host,
            port=port,
            path=parsed.path,
            qs=parsed.query,
    ...
    def _get_query(self) -> Mapping[str, List[str]]:
        """The query-string, parsed into a mapping of key -> [values, ...]."""
        return types.MappingProxyType(parse.parse_qs(self.qs) if self.qs else {})

    def _get_parameters(self) -> Mapping[str, List[str]]:
        """The params, parsed into a mapping of key -> [values, ...]."""
        return types.MappingProxyType(
            parse.parse_qs(self.params) if self.params else {}
        )
