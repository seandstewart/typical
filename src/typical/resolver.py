...
class Resolver:
    ...
    _DYNAMIC = serde.SerFactory._DYNAMIC
    OPTIONALS = (None, ..., type(None), type(...))
    LITERALS = (int, bytes, str, bool, Enum, type(None))

    def __init__(self):
        self.des = serde.DesFactory(self)
        self.ser = serde.SerFactory(self)
        self.binder = serde.Binder(self)
        self.translator = serde.TranslatorFactory(self)
        self.bind = self.binder.bind
        self.constraints_factory = constr.factory
        self.__cache = {}
        self.__stack = set()
    ...
    def primitive(
        ...
        *,
        ...
        >>> import re
        >>> import dataclasses
        >>> typical.primitive("foo")
        'foo'
        >>> typical.primitive(("foo",))  # containers are converted to lists/dicts
        ['foo']
        >>> typical.primitive(datetime.datetime(1970, 1, 1))
        '1970-01-01T00:00:00'
        >>> typical.primitive(b"foo")
        'foo'
        >>> typical.primitive(ipaddress.IPv4Address("0.0.0.0"))
        '0.0.0.0'
        >>> typical.primitive(re.compile("[0-9]"))
        '[0-9]'
        >>> typical.primitive(uuid.UUID(int=0))
    ...
    def tojson(
        ...
        >>> typical.tojson(("foo",))
        b'["foo"]'
        >>> typical.tojson(datetime.datetime(1970, 1, 1))
        b'"1970-01-01T00:00:00"'
        >>> typical.tojson(b"foo")
        b'"foo"'
        >>> typical.tojson(uuid.UUID(int=0))
    ...
    def constraints(
        ...
        *,
        ...
        default: Hashable | Callable[[], ObjectT] | empty = empty,
        **config,
    ) -> constr.AbstractConstraintValidator[ObjectT]:
        cv = self.constraints_factory.build(
            annotation, nullable=nullable, name=name, cls=cls, default=default, **config
        )
        return cv
    ...
    def _get_configuration(self, origin: Type, flags: SerdeFlags) -> SerdeConfig:
        ...
        params = inspection.safe_get_params(origin)
        # This is probably a builtin and has no signature
        fields: Dict[str, Annotation] = {}
        hints = inspection.cached_type_hints(origin)
        include = (
            flags.fields
        ...
        if omit:
            ...
            type_name_omissions = {inspection.get_name(o) for o in type_omissions}
            ...
            for name, out in fields_out.items():
                ...
                if isinstance(anno, ForwardDelayedAnnotation):
                    if (
                        not {
                            inspection.get_name(anno.ref),
                            inspection.get_name(default),
                        }
                        & type_name_omissions
                    ):
    ...
    def annotation(
        ...
        non_super = inspection.resolve_supertype(annotation)
        # Note, this may be a generic, like Union.
        orig = inspection.origin(annotation)
        use = non_super
        ...
        if is_optional is None:
            is_optional = (
                checks.isoptionaltype(non_super)
                or parameter.default in self.OPTIONALS
                or annotation in self.OPTIONALS
            )
        is_strict = is_strict or checks.isstrict(non_super) or self.STRICT
        is_static = inspection.origin(use) not in self._DYNAMIC
        ...
        while checks.should_unwrap(use) and args:
            ...
            non_super = inspection.resolve_supertype(args[0])
            use = non_super
            args = inspection.get_args(use)
            is_static = inspection.origin(use) not in self._DYNAMIC
            is_literal = is_literal or checks.isliteral(use)

        # Treat ellipses as 'null'
        if use in (..., type(...)):
            use = type(None)
        ...
        # The type definition is recursive or within a recursive loop.
        elif use is namespace or orig in self.__stack:
            # If detected via stack, we can remove it now.
            # Otherwise we'll cause another recursive loop.
            if orig in self.__stack:
                self.__stack.remove(orig)
            da = DelayedAnnotation(
                ...
                flags=fflags,
                default=default,
                module=use.__module__,
                frame=inspect.currentframe(),
            )
            return cast(Annotation, da)
        # Otherwise, add this type to the stack to prevent a recursive loop from elsewhere.
        is_stdlib = checks.isstdlibtype(orig)
        is_noop = use in (typing.Any, constants.empty)
        if (is_stdlib, is_noop) == (False, False):
            self.__stack.add(orig)
        serde = (
            self._get_configuration(inspection.origin(use), fflags)
    ...
    def _finalize_deserializer(
        ...
        if isinstance(
            contraint_config, (constr.TypeConstraints, constr.EnumerationConstraints)
        ):
            if isinstance(contraint_config, constr.EnumerationConstraints):
                d = deserializer
        ...
        if annotation.strict and isinstance(
            contraint_config, constr.StructuredObjectConstraints
        ):
            d = deserializer
    ...
    def _resolve_from_annotation(
        ...
        *,
        ...
        constraints = self.constraints(
            anno.resolved,
            nullable=anno.optional,
            cls=namespace,
            default=anno.parameter.default,
        )
    ...
    def _build_protocol(
        ...
        *,
        ...
        constraints: constr.AbstractConstraintValidator[ObjectT],
        deserializer: DeserializerT[ObjectT],
        validator: constr.validators.ValidatorProtocol[ObjectT],
    ...
    def protocols(
        ...
        ...     bar: str
        ...
        >>> protocols = typical.protocols(Foo)
        ...
        hints = inspection.cached_type_hints(obj)
        # "return" is an invalid name for a param, so we should skip it.
        hints.pop("return", None)
        params = inspection.safe_get_params(obj)
        ...
        for name in keys:
            ...
            annotation = inspection.resolve_supertype(annotation)
            ...
            if (
                ...
                if (
                    field.init is False
                    and inspection.origin(annotation) is not ReadOnly
                ):
