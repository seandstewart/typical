...
from typical.constraints.core import assertions, error
from typical.constraints.number import assertions as number
...
def get_assertion_cls(
    *,
    ...
    tuple[
        type[AbstractDecimalAssertion],
        type[number.AbstractNumberAssertion | assertions.NoOpAssertion],
    ]
    | None
):
...
class AbstractDecimalAssertion(assertions.AbstractAssertions[_DT]):
    ...
    def __init__(
        ...
        *,
        ...
        self.max_whole_digits = max_digits
        if None not in (max_digits, max_decimal_places):
            self.max_whole_digits = max_digits - max_decimal_places
        self._check_syntax()
        super().__init__()

    def _check_syntax(self):
        has_max_digits = self.max_digits is not None
        has_max_decimals = self.max_decimal_places is not None
        invalid_declaration = (
            has_max_decimals
            and has_max_digits
            and (self.max_digits < self.max_decimal_places)
        )
