...
from typical.core.constraints.core import error, validators
...
class AbstractConstraints(Generic[_VT]):
    ...
    def __post_init__(self):
        if self.type_name is None:
            object.__setattr__(self, "type_name", inspection.get_name(self.type))
        if self.type_qualname is None:
            object.__setattr__(
                self, "type_qualname", inspection.get_qualname(self.type)
            )
        if self.origin is None:
            object.__setattr__(self, "origin", inspection.origin(self.type))
...
_ET = TypeVar("_ET", enum.EnumMeta, Literal[...])  # type: ignore[valid-type,misc]
