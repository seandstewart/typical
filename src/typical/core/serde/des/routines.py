...
class BaseDeserializerRoutine(
    ...
    def _get_closure(self) -> DeserializerT[_T]:
        ...
        def deserializer(
            val, *, __check=check, __deserialize=deser, __evaluate=evaluate
        ) -> _T:
            value, valid = __check(val)
            if valid:
                return value
            evaluated = __evaluate(value)
            return __deserialize(evaluated)
    ...
    def _get_evaluate(self) -> _EvaluateT:
        ...
        def evaluate(o, *, __fromstr=desers.fromstr):
...
class MappingDeserializerRoutine(BaseDeserializerRoutine[Mapping[_KT, _VT]]):
    ...
    def _get_default_factory(self, annotation: Annotation = None):
        ...
        if not checks.isbuiltinsubtype(use):  # type: ignore
            ...
            params: Mapping[str, inspect.Parameter] = inspection.cached_signature(
                use
            ).parameters
...
class UnionDeserializerRoutine(BaseDeserializerRoutine[_T]):
    def _get_deserializer(self) -> DeserializerT[_T]:
        ...
        tagged = inspection.get_tag_for_types(targets)
    ...
    def _get_tagged_union_deserializer(
        self, tagged: inspection.TaggedUnion
    ) -> DeserializerT[_T]:
        _desers = {
