...
from typic.compat import ForwardRef, Generic, Protocol, TypedDict, evaluate_forwardref
from typic.core import constants
from typic.core import constraints as const
from typic.core import strict as st
from typic.core import strings
from typic.core.annotations import AnyOrTypeT, ObjectT, OriginT
...
class SerdeProtocol(Generic[OriginT]):
    ...
    constraints: Optional[const.AbstractConstraintValidator[OriginT]]
    ...
    validate: const.validators.ValidatorProtocol[OriginT] = dataclasses.field(
        repr=False
    )
...
class SerdeFlags:
    ...
    case: Optional[strings.Case] = None
    ...
    def __init__(
        ...
        *,
        ...
        self.exclude = types.freeze(exclude) or ()  # type: ignore[call-overload]
    ...
    def merge(self, other: "SerdeFlags") -> "SerdeFlags":
        ...
        if other.fields and self.fields:
            if not isinstance(other.fields, Mapping):
                other.fields = types.freeze({x: x for x in other.fields})  # type: ignore
            if not isinstance(self.fields, Mapping):
                self.fields = types.freeze({x: x for x in self.fields})  # type: ignore
...
class SerdeConfigD(TypedDict):
    fields: Mapping[str, Annotation | DelayedAnnotation | ForwardDelayedAnnotation]
